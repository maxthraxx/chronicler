<script lang="ts">
    ...
    $effect(() => {
        ...
        // Define the keys to exclude from the main infobox list.
        const excludedKeys = ['title', 'tags', 'infobox', 'image', 'error', 'details'];

        try {
            // Filter the frontmatter data to get only the fields we want to display.
            ...
            <dd>
                {#if Array.isArray(value)}
                    <ul>
                        {#each value as item}
                            <!--
                                THE FIX: The renderValue function is gone. We use @html directly
                                because the backend has already processed the wikilinks into HTML.
                            -->
                            <li>{@html item}</li>
                        {/each}
                    </ul>
...
<style>
    ...
    /* This global style is now handled by the parent Preview.svelte's click handler,
        so we can remove the specific `data-link-target` logic.
    */
    :global(.infobox a) {
